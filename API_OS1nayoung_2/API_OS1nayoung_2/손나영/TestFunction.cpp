...
BOOL test_Beep(){
	...
	wresult(__FILE__, __LINE__, "Beep", buf, "1", meg);
...
BOOL test_SetHandleCount(){
	...
	sprintf(buf, "%d", wresult_value);
	wresult(__FILE__, __LINE__, "SetHandleCount", buf, "1", meg);
...
BOOL test_SetPriorityClass(){
	...
	sprintf(buf, "%d", wresult_value);
	wresult(__FILE__, __LINE__, "SetPriorityClass", buf, "1", meg);
...
BOOL test_GetPriorityClass(){
	...
	sprintf(buf, "%d", wresult_value);
	wresult(__FILE__, __LINE__, "GetPriorityClass", buf, "1", meg);
	return true;
}
